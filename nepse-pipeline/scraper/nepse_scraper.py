...
⚠️  SELECTOR DISCLAIMER
This was written without live access to nepsealpha.com or nepalstock.com.np
(sandboxed network, see README). The CSS/XPath selectors below are
best-effort placeholders based on typical floorsheet table structures.
Before relying on this, run:

    python -m scraper.nepse_scraper --once --debug

`--debug` saves a screenshot + the full page HTML to ./debug_output/ so you
can open dev tools, find the *real* selectors, and fix the few lines marked
`# TODO: VERIFY SELECTOR`. This is genuinely a 10-15 minute fix, not a
rewrite — the surrounding logic (pagination, retry, rate limiting, output
schema) doesn't need to change.

Usage:
    python -m scraper.nepse_scraper --once            # one manual run
    python -m scraper.nepse_scraper --once --debug     # + save HTML/screenshot
    python -m scraper.nepse_scraper --date 2026-06-22  # scrape a specific date
"""
