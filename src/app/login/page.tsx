...
function LoginPageContent() {
  ...
  // Fallback click handler if Google button didn't render
  function onGoogleClick() {
    setError(null);
    // @ts-ignore
    const google = window.google;
    if (!google?.accounts?.id) {
      showToast("Google Sign-In not ready. Please refresh.", "error");
      return;
    }
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[#f1f5f9] p-4">
      ...
      <div className="w-full max-w-[420px] rounded-2xl bg-white p-10 md:p-12 text-center shadow-[0_20px_60px_rgba(0,0,0,0.12)]">
        ...
        {/* Fallback button if Google button didn't render */}
        {!googleReady && (
          <button
            onClick={onGoogleClick}
            className="w-full rounded-lg border border-[#d1d5db] bg-white py-3 px-4 text-base font-medium text-[#374151] flex items-center justify-center gap-3 transition-all duration-200 hover:bg-[#f9fafb] hover:border-[#9ca3af]"
          >
